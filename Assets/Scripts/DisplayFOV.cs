...
public class DisplayFOV : MonoBehaviour
{
    ...
    float m_meshResolution = 1;
    int m_edgeResolveIterations = 4;
    float m_edgeDstThreshold = 0.5f;
    ...
    void Start()
    {
        ...
        m_viewMeshFilter = (MeshFilter)go.AddComponent(typeof(MeshFilter));
        MeshRenderer renderer = (MeshRenderer)go.AddComponent(typeof(MeshRenderer));

        if (FOVMat)
        {
            renderer.material = FOVMat;
        }
    ...
    {
        int stepCount = Mathf.RoundToInt(viewAngle * m_meshResolution);
